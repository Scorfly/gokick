...
func TestGetAuthorizeSuccess(t *testing.T) {
	kickClient, err := gokick.NewClient(&gokick.ClientOptions{
		ClientID: "client-id",
	})
	require.NoError(t, err)

	response, err := kickClient.GetAuthorize(
...
func TestGetTokenError(t *testing.T) {
	t.Run("client ID missing", func(t *testing.T) {
		...
		_, err = kickClient.GetToken(ctx, "redirectURI", "code", "codeVerifier")
...
func TestGetTokenSuccess(t *testing.T) {
	kickClient := setupMockAuthClient(t, func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(t, "redirectURI", r.FormValue("redirect_uri"))
		assert.Equal(t, "codeVerifier", r.FormValue("code_verifier"))
	...
	response, err := kickClient.GetToken(context.Background(), "redirectURI", "code", "codeVerifier")
	require.NoError(t, err)
...
func TestGetAppAccessTokenError(t *testing.T) {
	t.Run("client ID missing", func(t *testing.T) {
		kickClient, err := gokick.NewClient(&gokick.ClientOptions{
			ClientSecret: "client-secret",
		})
		...
		var ctx context.Context
		_, err = kickClient.GetAppAccessToken(ctx)
		require.EqualError(t, err, "client ID must be set on Client to refresh token")
	})
	...
	t.Run("timeout", func(t *testing.T) {
		...
		_, err := kickClient.GetAppAccessToken(context.Background())
		require.EqualError(t, err, `failed to make request: Post "https://id.kick.com/oauth/token": context deadline exceeded `+
			`(Client.Timeout exceeded while awaiting headers)`)
	})
	...
	t.Run("unmarshal token response", func(t *testing.T) {
		...
		assert.EqualError(t, err, `failed to unmarshal response body (KICK status code 200 and body "117"): json: cannot unmarshal `+
			`number into Go value of type gokick.AppTokenResponse`)
	})
...
func TestGetAppAccessTokenSuccess(t *testing.T) {
	kickClient := setupMockAuthClient(t, func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, http.MethodPost, r.Method)
		assert.Equal(t, "/oauth/token", r.URL.Path)
		assert.Equal(t, "application/x-www-form-urlencoded", r.Header.Get("Content-Type"))
	...
	response, err := kickClient.GetAppAccessToken(context.Background())
	require.NoError(t, err)
	assert.Equal(t, "access-token", response.AccessToken)
	assert.Equal(t, 7200, response.ExpiresIn)
	assert.Equal(t, "Bearer", response.TokenType)
